...
const getTokensCacheKey = (code: string, language: BundledLanguage) => {
  const source = code ?? "";
  const start = source.slice(0, 100);
  const end = source.length > 100 ? source.slice(-100) : "";
  return `${language}:${source.length}:${start}:${end}`;
};
  ...
  return (
    <CodeBlockContext.Provider value={contextValue}>
      <CodeBlockContainer className={className} language={language} {...props}>
        {children}
        <CodeBlockContent code={source} language={language} showLineNumbers={showLineNumbers} />
      </CodeBlockContainer>
